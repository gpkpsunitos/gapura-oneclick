...
export async function GET(request: NextRequest) {
  try {
    ...
    const fetchAnalyzeAllPayload = async () => {
    ...
    const aiScope = {
      ...
      includeTrends,
      esklasiRegex,
      branch: branchCode || branchFilter || null,
      division: divisionParam || null,
      role,
    };

    const resolved = bypassCache.toLowerCase() === 'true'
      ? {
          payload: await fetchAnalyzeAllPayload(),
          cached: false,
      ...
      : await resolveCachedAI({
          feature: 'analyze-all',
          scope: aiScope,
          resolver: fetchAnalyzeAllPayload,
        });
    ...
    return NextResponse.json({
      ...
      sourceSyncAt: resolved.sourceSyncAt,

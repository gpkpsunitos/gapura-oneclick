...
import {
  ...
  Activity, Clock, Gauge, Zap, Shield, BarChart3, Building2, Wrench, User as UserIcon,
...
export function DivisionAIReportsDashboard({ division = 'OS', branchFilter }: DivisionAIReportsDashboardProps) {
  ...
  const prioritized = useMemo(() => {
    ...
    return rows
      .map((r) => {
        const sev = (r.classification?.severity || 'LOW').toUpperCase();
  ...
  const recommendationGroups = useMemo(() => {
    ...
    prioritized.forEach(({ item }) => {
      const sev = (item.classification?.severity || 'LOW').toUpperCase();
                      ...
                      {[
                        ...
                        <div key={i} className="p-2.5 rounded-xl border border-gray-100 bg-gray-50/50">
                          <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-tight">
                            <it.icon className="w-3 h-3 text-gray-400" /> {it.label}
                          </div>
                          <div className="text-xs font-semibold text-gray-700 mt-0.5">{it.value}</div>
                        </div>
                      ) : null)}
                    </div>
                  </div>
                      ...
                      {[
                        ...
                        <div key={i} className="p-3 rounded-xl border border-gray-200 bg-white shadow-sm">
                          <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                            <it.icon className="w-3.5 h-3.5 text-gray-400" /> {it.label}
                          </div>
                          <div className="text-[13px] font-medium text-gray-700 mt-1.5 leading-relaxed">{it.value}</div>
                        </div>
                      ) : null)}
                    </div>
                  </div>

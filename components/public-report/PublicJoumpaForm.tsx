...
export default function PublicJoumpaForm({
  ...
  const valid = useMemo(() => {
    const base = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.reporter_email.trim()) &&
      form.report_by.trim() && form.date_of_event && resolveOther(form.airlines, form.airlines_other) &&
      form.flight_number.trim() && form.station_id && form.route.trim() && form.category_report &&
      form.customer_joumpa && form.category_case_joumpa && form.report.trim() && form.root_caused.trim() &&
      form.action_taken.trim() && form.preventive_action.trim() && form.severity_level && form.status &&
      files.length > 0;
    ...
    return false;
  }, [form, files, isCorporate, isNonCorporate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        airlines: resolveOther(form.airlines, form.airlines_other),
  ...
  const successNode = (
    <div className="jm-success">
      ...
      <p className="jm-success__body">Your JOUMPA entry has been recorded.</p>
      {onClose && <button type="button" onClick={onClose} className="jm-submit jm-submit--success">Done</button>}
      {!onClose && inline && (
        <button type="button" onClick={() => { setSuccess(false); setForm({ ...EMPTY, report_by: defaultReporterName || '', reporter_email: defaultReporterEmail || '' }); setFiles([]); }} className="jm-submit jm-submit--success">File another</button>
      )}
    </div>
  );
  ...
  return (
    <FormShell onClose={onClose ?? (() => {})} ariaLabel="JOUMPA Report">
      {body}
    </FormShell>
  );
}

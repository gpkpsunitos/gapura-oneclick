...
const COLUMN_CANDIDATES = {
  ...
  branch: ['Branch','Cabang','Reporting_Branch','Reporting Branch'],
  route: ['Route','Rute'],
...
function parseRow(raw, colMap, sheetName, rowIndex) {
  ...
  const evidenceUrls = evidenceRaw ? evidenceRaw.split(/\s*\|\s*|\n+/).map(s => s.trim()).filter(Boolean) : null;
  ...
  const mainCat = normalizeCategory(get('main_category') || get('irregularity_complain_category'));
